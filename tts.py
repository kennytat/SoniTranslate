...
def text_to_speech(duration_net, generator, text, model_path, hps, speed, max_word_length=750):
...
def synthesize(output_dir_name, input, is_file, speed, method, tts_voice_ckpt_dir, convert_voice_ckpt_dir):
    ...
    if torch.cuda.is_available() and convert_voice_ckpt_dir != "none":
      convert_voice(tmp_dirname, convert_voice_ckpt_dir)
  ...
  method="join"
    ...
    if input_text:
      try:
          ...
          output_temp_file, log_temp_file = synthesize(output_dir_name, input_text, False, speed, method, tts_voice_ckpt_dir, convert_voice_ckpt_dir)
          if log_temp_file:
    ...
    if input_files:
      ...
      for file_path in file_list:
          try:
              ...
              output_temp_file, log_temp_file = synthesize(output_dir_name, file_path, True, speed, method, tts_voice_ckpt_dir, convert_voice_ckpt_dir)
              results_list.append(output_temp_file)
              if log_temp_file:
                logs_list.append(log_temp_file)
          except:
